...
const panels=new Set(['beranda','berita','pustaka','kta','arsip','suara','kontak','quran']);
let busyUntil=0;
...
function activateBeritaTab(name){
 ...
 if(b){const prev=window.__PCM_INTERNAL_CLICK__;window.__PCM_INTERNAL_CLICK__=true;b.click();window.__PCM_INTERNAL_CLICK__=prev;}
}
function mountQuran(){
 let sec=qs('#quran');
 if(!sec){const main=qs('main');if(!main)return null;sec=document.createElement('section');sec.id='quran';sec.className='panel';main.appendChild(sec)}
 if(!window.__PCM_QURAN_SCRIPT__){window.__PCM_QURAN_SCRIPT__=true;const s=document.createElement('script');s.src='quran-reader.js';s.defer=true;s.onload=()=>window.PCMQuran?.mount();document.body.appendChild(s)}else window.PCMQuran?.mount();
 return sec;
}
function ensureQuranMenu(){
 const parent=qs('.nav-drop .nav-parent[href="#pustaka"]');
 if(!parent)return;
 const menu=parent.parentElement.querySelector('.submenu');
 if(menu&&!menu.querySelector('a[href="#quran"]')){const a=document.createElement('a');a.href='#quran';a.textContent="Al-Qur'anMu";menu.prepend(a)}
}
function handleLink(link){
 const href=link.getAttribute('href')||'';if(!href.startsWith('#'))return false;
 const id=href.slice(1);const drop=link.closest('.nav-drop');
 const parentId=drop?((drop.querySelector('.nav-parent')?.getAttribute('href')||'').slice(1)):'';
 if(id==='quran'){mountQuran();activatePanel('quran');window.scrollTo(0,0);return true}
 if(drop&&parentId==='berita'&&['profil','kajian','agenda','struktur'].includes(id)){activateBeritaTab(id);return true}
 const dest=drop&&panels.has(parentId)?parentId:id;
 if(dest==='berita')activatePanel('berita');else if(panels.has(dest))activatePanel(dest);else return false;
 return true;
}
function polish(){
 if(qs('#pcm-fast-ui'))return;
 const s=document.createElement('style');s.id='pcm-fast-ui';s.textContent=`
 ...
 @media(max-width:560px){.nav-inner{gap:6px!important}.nav-item{padding:9px 10px!important;font-size:.88rem!important}.berita-head{margin-left:2px;margin-right:2px}.berita-tabs{overflow-x:auto;justify-content:flex-start!important;flex-wrap:nowrap!important;scrollbar-width:none;padding-left:3px!important;padding-right:3px!important}.berita-tabs::-webkit-scrollbar{display:none}.berita-tab{flex:0 0 auto}.bm-card,.org-node,.org-unit{box-shadow:0 9px 22px rgba(7,61,120,.08)}}
 `;document.head.appendChild(s);
}
function install(){
 polish();ensureQuranMenu();
 window.addEventListener('pointerdown',e=>{if(e.button!==undefined&&e.button!==0&&e.pointerType==='mouse')return;const link=e.target.closest?.('a[href^="#"]');if(!link)return;if(handleLink(link)){e.preventDefault();e.stopImmediatePropagation();busyUntil=performance.now()+450}},true);
 window.addEventListener('click',e=>{if(window.__PCM_INTERNAL_CLICK__)return;const link=e.target.closest?.('a[href^="#"]');if(link&&performance.now()<busyUntil){e.preventDefault();e.stopImmediatePropagation();return}if(link&&handleLink(link)){e.preventDefault();e.stopImmediatePropagation()}},true);
 window.addEventListener('pointerdown',e=>{const b=e.target.closest?.('#bmTabs [data-bm]');if(!b)return;e.preventDefault();e.stopImmediatePropagation();const prev=window.__PCM_INTERNAL_CLICK__;window.__PCM_INTERNAL_CLICK__=true;b.click();window.__PCM_INTERNAL_CLICK__=prev;busyUntil=performance.now()+450},true);
 window.addEventListener('click',e=>{if(window.__PCM_INTERNAL_CLICK__)return;const b=e.target.closest?.('#bmTabs [data-bm]');if(b){e.preventDefault();e.stopImmediatePropagation()}},true);
 if(location.hash==='#quran'){mountQuran();activatePanel('quran')}
}
