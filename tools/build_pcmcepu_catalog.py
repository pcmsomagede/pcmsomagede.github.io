...
S=requests.Session(); S.headers['User-Agent']='PCM-Somagede-Content-Catalog/2.0'
...
def category(u,text=''):
    ...
    if 'kajian' in s or 'kajian' in text.lower():return 'kajian'
    if 'pustaka' in s or 'download' in s or '.pdf' in s or '.doc' in s:return 'pustaka'
    return 'lainnya'

def sitemap_urls():
    out=[]
    candidates=[BASE+'sitemap.xml',BASE+'atom.xml?redirect=false&start-index=1&max-results=500']
    for u in candidates:
        try:
...
def parse_page(u):
    try:
        ...
        for sel in ('time[datetime]','meta[itemprop="datePublished"]','abbr.published'):
            ...
            if el:
                published=el.get('datetime') or el.get('content') or el.get_text(' ',strip=True);break
        txt=soup.get_text(' ',strip=True)
        cat=category(u,txt[:6000])
        rows=[{'title':title,'description':desc,'url':u,'category':cat,'published':published}]
        if u==PUSTAKA:
            for a in soup.find_all('a',href=True):
                ...
                if urlparse(x).netloc.endswith('pcmcepu.com') and ('pdf' in x.lower() or 'doc' in x.lower()):
                    rows.append({'title':label,'description':'Materi pustaka digital PCM Cepu.','url':x,'category':'pustaka','published':''})
        return rows
    except Exception:return []

def main():
    urls=[PUSTAKA]+sitemap_urls()
    # Prioritise the content families the PCM Somagede library needs.
    preferred=[]
    for u in urls:
        s=u.lower()
        if any(k in s for k in ('kajian','kultum','khutbah','pustaka','alquran','al-quran')) or u==PUSTAKA:
            preferred.append(u)
    # Keep recent/general article pages as a secondary source.
    urls=list(dict.fromkeys(preferred+urls))[:MAX_PAGES]
    rows=[]
    with ThreadPoolExecutor(max_workers=WORKERS) as ex:
        futs=[ex.submit(parse_page,u) for u in urls]
        for f in as_completed(futs):rows.extend(f.result())
    by={x['url']+'|'+x['title']:x for x in rows if x.get('title')}
    rows=sorted(by.values(),key=lambda x:(x.get('published',''),x['title']),reverse=True)
    os.makedirs('data',exist_ok=True)
    payload={'source':'PCM Cepu','source_home':BASE,'generated':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'total':len(rows),'items':rows}
    with open(OUT,'w',encoding='utf-8') as f:json.dump(payload,f,ensure_ascii=False,indent=2)
    print('PCM Cepu catalog:',len(rows),'items from',len(urls),'pages')
