#!/usr/bin/env python3
# PustakaMu catalog builder v2: scalable grouping, covers, and DOCX derivatives.
...
def upload(file_or_url,title,resource_type,public_id):
    ...
    if os.path.isfile(str(file_or_url)):
        ...
        r=S.post(endpoint,data={'upload_preset':PRESET,'file':file_or_url,'public_id':public_id,'tags':'pustaka_somagede','context':f'title={title}'},timeout=180)
...
def main():
    ...
    with tempfile.TemporaryDirectory() as td:
        for n,g in enumerate(groups.values(),1):
            ...
            try:
                if g.get('pdf_source'):
                    if prior.get('cloudinary_pdf_url'):rec['pdf_url']=prior['cloudinary_pdf_url']
                    elif prior.get('cloudinary_url'):rec['pdf_url']=prior['cloudinary_url']
                    else:
                        r=upload(g['pdf_source'],title,'image',base);rec['pdf_url']=r.get('secure_url') or r.get('url')
                    rec['cloudinary_url']=rec.get('pdf_url');rec['status']='ready'
                    rec['cover_url']=re.sub(r'\.pdf(?=($|[?#]))','.jpg',rec.get('pdf_url',''),flags=re.I)
                if g.get('docx_source'):
                    if prior.get('cloudinary_docx_url'):rec['docx_url']=prior['cloudinary_docx_url']
                    else:
                        r=upload(g['docx_source'],title,'raw',base+'.docx');rec['docx_url']=r.get('secure_url') or r.get('url')
                    ...
                    try:
                        ...
                        r=upload(docx_path,title,'raw',base+'.docx');rec['docx_url']=r.get('secure_url') or r.get('url');rec['docx_generated']=True
